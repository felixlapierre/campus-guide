...
abstract class IndoorPathfinding constructor(private val graph: Graph) {
    ...
    open fun findRoom(start: String): List<List<Node>> {
        if (graph.get(start) == null) {
            throw NonexistentLocationException("$start ${Constants.LOCATION_NOT_FOUND_GRAPH_MSG}")
        }
    ...
    private fun iterate() {
        ...
        currNode.edges.forEach {
            val neighbor = graph.get(it)
                ?: throw NonexistentLocationException("${Constants.LOCATION_NOT_FOUND_ROOM_MSG} $it")
